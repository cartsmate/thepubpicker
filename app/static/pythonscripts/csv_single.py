...
class CsvSingle:
    ...
    def go_get_1_data(self, pub_id):
        df_pub = CsvSingle().go_get_1_pub(pub_id)
        print(df_pub)
        df_review = CsvSingle().go_get_1_review(pub_id)
        df_pb_rev = pd.merge(df_pub, df_review, on='pub_identity', how='left')

        station_id = df_pub['station_identity'].iloc[0]
        df_station = self.go_get_1_station(station_id)
        df_pb_rev_st = pd.merge(df_pb_rev, df_station, on='station_identity', how='left')

        direction_id = df_station['direction_identity'].iloc[0]
        df_direction = self.go_get_1_direction(direction_id)
        df_pb_rev_st_dir = pd.merge(df_pb_rev_st, df_direction, on='direction_identity', how='left')
