...
class EditPub:
    ...
    def go_edit_pub(self, id):
        # new_pub_id = UuidGenerator().get_new_uuid()
        df_detail = NewDetail().go_get_new_detail(pub_id=id)
        df_review = NewReview().go_get_new_review(id=UuidGenerator().get_new_uuid(), pub_id=id)
        ...
        df_diary = NewDiary().go_get_new_diary(pub_id=id)
        df_pb_rev_st_dir_dry = pd.merge(df_pb_rev_st_dir, df_diary, on='pub_identity', how='left')

        df_photo = NewPhoto().go_get_new_photo(id=UuidGenerator().get_new_uuid(), pub_id=id)
